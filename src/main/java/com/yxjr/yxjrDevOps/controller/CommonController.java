...
import com.yxjr.yxjrDevOps.service.TaskInfoService;
...
public class CommonController extends ApiController {
     ...
     * @throws UnsupportedEncodingException
     */
    @RequestMapping("/Log/{logTime}/{logType}")
    @ResponseBody
    public String downloadLog(HttpServletRequest request,
                                  HttpServletResponse response,@PathVariable(name = "logTime")String logTime ,@PathVariable(name = "logType")String logType) throws UnsupportedEncodingException {
        logger.info("设备开始下载服务器日志");
        String FullPath ="";
        if("info".equals(logType)){
            FullPath="/home/sysmon/UPDATE/yxjrApp/log/yxgdjh-log."+logTime+".0.log";
        }else if("error".equals(logType)){
            FullPath="/home/sysmon/UPDATE/yxjrApp/log/yxgdjh-log_error."+logTime+".0.log";
        }else{
            String s = "{\"code\":-2,\"msg\":\"无效下载类型\"}";
            return s;
        }
        File packetFile = new File(FullPath);
        String fileName = packetFile.getName(); //下载的文件名
        ...
        if (packetFile.exists()) {
            ...
            try {
                ...
                logger.info("下载服务器日志成功");
            } catch (Exception e) {
                logger.error("下载服务器日志成功"+e);
            } finally {
                if (bis != null) {
                    try {
                        bis.close();
